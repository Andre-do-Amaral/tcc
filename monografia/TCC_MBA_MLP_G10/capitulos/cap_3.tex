...
Os dados são obtidos via \acrfull{API} no site \url{https://mananciais.sabesp.com.br/api/v4} do \acrshort{SABESP}, esses dados são populados diariamente em uma base de consulta pública com informações da   situação nos níveis, vazões, precipitação e demais dados inerentes de cada reservatório individualmente, bem como do \acrfull{SCE}, que retrata o sistema como um todo. O \acrshort{SCE}, basicamente, é uma agregação dos valores individuais dos reservatórios e suas vazões de tal forma que representem um modelo de sistema um pouco menos complexo. E são esses dados do \acrshort{SCE} que foram utilizados neste estudo.

Para as requisições no site do \acrshort{SABESP} enviamos parâmetros para designar o que queremos extrair, são eles: Data de Início; Data Fim; Número que indica o reservatório ou \acrshort{SCE} (em nosso estudo, utilizamos para o \acrshort{SCE}). O retorno dessa requisição é um \acrfull{JSON} com todas as informações sobre o \acrshort{SCE} para a data determinada que estamos fazendo a coleta.
...
Outra análise realizada foi quanto à sazonalidade da variável alvo, utilizando o periodograma, que tem por objetivo identificar ciclos repetitivos nos dados. A \autoref{fig:Fig_6}, confirma que existe de fato uma periodicidade anual nos dados como havia sido interpretado na decomposição da série temporal na \autoref{fig:Fig_5}, além disso, podemos observar que há outras periodicidades como tetra ou octo anual, que podem estar relacionados a eventos climáticos que influenciaram na chegada de água do reservatório causando períodos de extrema seca ou picos de cheia dos reservatórios. Por exemplo, no gráfico da \autoref{fig:Fig_3}, é possível visualizar a ocorrência de valores extremos nos dados numa frequência bem maior do que anual.  
...
Outro aspecto fundamental de uma série temporal que foi analisado, é a relação da variável com os seus valores anteriores, visto que a modelagem de uma série temporal tem, entre outros objetivos, aprender comportamentos com os dados do passado, para prever cenários futuros \cite{Nielsen}. A \autoref{fig:Fig_7} a seguir, apresenta os gráficos de \acrfull{ACF} e \acrfull{PACF} da variável alvo. Observa-se que a autocorrelação permanece elevada mesmo para defasagens longas, indicando que a variável alvo apresenta forte dependência temporal e elevada inércia, variando de forma gradual.  Nota-se que no dia 1 a autocorrelação é 1, e no dia 60 ainda está muito próximo de 1. 

A análise de autocorrelação parcial mostra que a primeira defasagem (lag 1) possui correlação significativa, enquanto as defasagens subsequentes apresentam queda abrupta dos valores de \acrshort{PACF}. Esse comportamento indica que o volume útil em um instante depende fortemente de seu valor imediatamente anterior, enquanto a contribuição de defasagens mais longas é significativamente menor. Dessa forma, o volume útil pode ser caracterizado como uma variável com alta persistência temporal, o que justifica o uso de defasagem da própria série como variáveis endógenas nos modelos de previsão, em especial o lag 1, que se mostra relevante.  
...
Analogamente, nesses gráficos de \acrshort{ACF} e \acrshort{PACF} da vazão jusante, o lag 1 e, talvez lag2, fazem sentido serem utilizados para realizar previsões na série temporal de vazão jusante.  
...
Através da análise do gráfico da \autoref{fig:Fig_24}, notamos que a vazão natural e a chuva de fato têm um comportamento parecido, quando a chuva é alta, as vazões naturais são altas, o que sugere que a chuva tem uma ação de curto prazo sobre a vazão natural. Ainda podemos observar que, a ocorrência de valores altos na vazão natural e chuva precede o aumento do volume útil, quando há picos nessas variáveis a tendência do volume é subir, e quando os valores são baixos logo em seguida o volume começa a cair. No entanto o efeito não é imediato, embora a direção na tendência dos dados mude conforme muda o comportamento dessas variáveis exógenas, o volume não sobe abruptamente. Por fim, nota-se que ocorre o comportamento contrário quando comparados o volume útil com a vazão jusante, os picos de saída do reservatório, geralmente antecedem uma queda no volume.  Neste gráfico, ainda é possível notar que os picos de vazão jusante ocorreram exatamente nas altas de volume e vazão natural, o que é esperado também, uma vez que temos alta disponibilidade de água, a SABESP diminui tarifas e também disponibiliza mais água, o que faz aumentar o consumo (influenciando períodos de volume menor no futuro). 
...
Todas as variáveis numéricas foram submetidas a padronização (StandardScaler) para garantir a convergência adequada dos algoritmos baseados em distância (\acrshort{KNN}) e regularização (Ridge).
...
\begin{itemize}
\item \textbf{\acrfull{MAE}}: que mede o erro médio absoluto, fornecendo interpretação direta em unidades do volume e robustez relativa a erros extremos.
\item \textbf{\acrfull{RMSE}}: que mede a raiz do erro quadrático médio, penalizando mais fortemente grandes desvios e sendo útil para comparar configurações quando se deseja reduzir erros de maior magnitude.
\end{itemize}

Para cada combinação de modelos e hiperparâmetros do grid search, as métricas foram calculadas em todos os folds e agregadas por média, adotando-se o menor erro médio (com ênfase em \acrshort{RMSE} e apoio do \acrshort{MAE}) como critério para seleção da configuração final.

Outros modelos clássicos como \acrshort{ARIMA}/\acrshort{SARIMAX} foram considerados e avaliados, porém apresentaram desempenho inferior ao obtido pela estratégia de decomposição da vazão + modelagem do ciclo por ML + regressão do volume com variável exógena, especialmente no contexto de previsão recursiva. Por esse motivo, optou-se por concentrar o desenvolvimento e o refinamento dos modelos na abordagem híbrida descrita acima, que demonstrou melhor capacidade preditiva no conjunto de dados avaliado.

\section{Arquitetura}

Esta seção tem o objetivo de apresentar a arquitetura proposta e implementada em ambiente de nuvem \acrfull{AWS}. 

Houve preferência pela utilização de recursos de computação sem servidor a fim de reduzir custos, já que a pipeline de treinamento não necessita de máquinas dedicadas em funcionamento durante todo o tempo. 

A arquitetura foi estruturada em módulos independentes de coleta, treinamento, promoção de modelos e previsão, conforme ilustrado na \autoref{fig:Fig_25} a seguir. 

\begin{figure}[H]
	\centering
	\caption{Diagrama da Arquitetura da Aplicação.}
	\label{fig:Fig_25}
	\includegraphics[width=0.90\textwidth,keepaspectratio]{figuras/imagem25.png}
	\fonte{Elaborado pelos autores.}
\end{figure}

\subsection{Coleta de Dados}

A etapa de coleta de dados foi executada por meio de funções Lambda, que acessam a URL \url{"https://mananciais.sabesp.com.br/api/v4"} e armazenam os dados em um arquivo "\acrfull{CSV}", armazenado em um serviço "Amazon \acrfull{S3}". 

O serviço \acrshort{AWS} Lambda oferece um meio de executar código sem servidor, de maneira escalável. No cenário em questão, em que dados novos são produzidos diariamente, o código só precisa ser executado, no momento de fazer novas previsões ou treinamento de modelos.  

Já o serviço \acrshort{AWS} "\acrshort{S3}" é um tipo de armazenamento para arquivos, que não necessita de gerenciamento de servidores dedicados. O \acrshort{S3} possui o conceito de "bucket", que são o "espaço" de armazenamento, e que internamente podem conter pastas para organização.  

\subsection{MLFlow}

O Servidor \acrfull{MLflow} permite gerenciar de maneira centralizada todo o ciclo de vida de modelos de machine learning. É possível registrar treinamentos executados, parâmetros utilizados, métricas de resultado, artefatos utilizados e os modelos selecionados para produção.  

Para implementar o servidor \acrshort{MLflow} foi utilizado o serviço Amazon \acrfull{ECS}, além de um "Bucket \acrshort{S3}" para armazenamento de artefatos e um banco de dados Postgres para armazenamento de experimentos, resultados e métricas. 

O serviço Amazon \acrshort{ECS} é um serviço de orquestração de containers que permite executar aplicações em nuvem de forma gerenciada. A aplicação é empacotada como uma imagem de container, construída a partir de um arquivo "Dockerfile" e armazenada no \acrfull{ECR},  

No \acrshort{ECS}, essa imagem é referenciada por uma definição de tarefa (task definition), que descreve como o container deve ser executado: imagem utilizada, variáveis de ambiente, bem como definições de hardware. A definição de tarefa é um modelo de execução que descreve o container sem executá-lo. 

Já o serviço do \acrshort{ECS} é responsável por manter a execução dessa tarefa ao longo do tempo. Ele define quantas instâncias da tarefa devem estar ativas, gerencia a necessidade de reinicializações automáticas em caso de falha, integra-se com balanceadores de carga e possibilita escalabilidade. Portanto, o serviço garante a efetiva execução do container. 

Para o banco Postgres foi utilizado o AuroraDB, por ser autogerenciado. Este banco, se utilizado 24 horas, ficaria mais caro que uma solução "tradicional" (Amazon \acrfull{RDS}). Contudo, se o servidor \acrshort{MLflow} for desligado quando não utilizado, o número de conexões com o banco de dados cai para zero e o AuroraDB na versão "serverless v2" pode desligar todas suas instâncias, trazendo economia. 

O serviço \acrshort{ECS}, em cada execução tem IP de acesso diferente. Para poder acessar em uma única URL, foi necessário configurar um balanceador de carga ("Load Balancer"). 

Já para armazenamento de variáveis de ambiente, foi utilizado o "Armazem de Parâmetros", dentro do serviço "\acrshort{AWS} System Manager". 

\subsection{Pipeline de Treinamento e Previsão}

A execução da pipeline de treino também foi feita utilizando "Amazon \acrshort{ECS}", pela simplicidade em executar.  

O container que atua como pipeline foi projetado de forma idempotente, permitindo que o container execute um script Python responsável por decidir sobre a necessidade de retreino, registrar experimentos no \acrshort{MLflow}, recuperar o modelo em produção e persistir os resultados das previsões em um arquivo ".\acrshort{CSV}" no Amazon \acrshort{S3}. 

\subsection{Orquestração}

A cada ciclo de treino e previsão, é necessário o provisionamento temporário do servidor \acrshort{MLflow}, a coleta de dados, a execução da pipeline e o desligamento dos recursos ao final do processo. 

Para essa orquestração, foi utilizado o serviço "Amazon Step Functions" e sua execução foi acionada por meio do serviço "Amazon EventBridge". 

O serviço "Step Functions" permite gerenciar fluxos de trabalho definidas como máquinas de estado. A grande vantagem em sua utilização é a integração com outros serviços \acrshort{AWS}, o que o torna poderoso em um cenário de construção de software nativo em nuvem. 

O Amazon EventBridge atua como um “barramento de eventos”, ideal para aplicações orientadas a eventos. Para testes, a frequência de acionamento definida foi diária, mas considerando um horizonte de previsão de 90 dias, essa frequência, em produção, foi definida como semanal. 

O diagrama da função Step Function pode ser visto na \autoref{fig:Fig_26} a seguir. Os temporizadores mostrados na máquina de estados foram posicionados antes e depois da execução, com o intuito de esperar o servidor \acrshort{MLflow} ficar disponível e, após o término da execução, deixar o servidor ligado por tempo suficiente para que algum operador humano consiga verificar os resultados. 

\begin{figure}[H]
	\centering
	\caption{Máquina de Estados da Orquestração.}
	\label{fig:Fig_26}
	\includegraphics[width=0.50\textwidth,keepaspectratio]{figuras/imagem26.png}
	\fonte{Elaborado pelos autores.}
\end{figure}

\section{Resultados}

Em nossas análises e metodologia escolhida, optamos por utilizar modelo híbrido, entretanto, para essa modelagem ter resultados eficientes, é necessária uma boa engenharia de variáveis. Em nosso modelo, como explícitamos ao longo do trabalho, criamos algumas variáveis opcionais ao modelo que funcionaram como hiperparâmetros para seleção do melhor modelo (em atividades de retreino, essa calibração de hiperparâmetros é refeita para selecionar um novo modelo e finalizar o ciclo de vida do anterior), são eles:

\begin{itemize}
\item \textbf{Modelo de vazão}: seleciona o tipo de modelo entre KNN e Floresta Aleatória (RandomForest) para modelar o ciclo da vazão natural e da vazão jusante;
\item \textbf{Cycle logic}: seleciona entre adicionar ou não um fator de amortecimento no ciclo da vazão natural de modo a evitar que ela escale a valores altos e influencie num volume sempre crescente;
\item \textbf{Modelo de volume}: seleciona o tipo de modelo linear para o volume, entre a Regressão linear, a Ridge e a Lasso (de modo a avaliar se a regularização melhora o modelo). Note que aqui, como temos entradas aditivas (Vazão, tendência e sazonalidade), não se faz necessário um modelo não linear;
\item \textbf{Diff1}: seleciona se o modelo de volume usará como variável a diferença do volume atual pelo anterior;
\item \textbf{Diff2}: Seleciona se o modelo de volume usará como variável a diferença do volume anterior pelo anterior a ele;
\item \textbf{Use\_vol\_clim}: Seleciona se o modelo de volume, ao final, na previsão, usará climatologia ao invés da tendência e sazonalidade na adição ao ciclo de volume.
\end{itemize}

Para otimização do modelo, implementamos um Grid Search integrado a uma validação cruzada temporal. O processo consistiu em 10 rodadas de teste para cada configuração: iniciou-se com uma base histórica, prevendo os 90 dias seguintes; sucessivamente, a base de treino foi expandida em blocos de 90 dias para novas previsões. O desempenho global de cada parâmetro foi mensurado pela média do Erro Quadrático Médio (\acrshort{RMSE}) e do Erro Médio Absoluto (\acrshort{MAE}), sendo o \acrshort{RMSE} o decisor para a escolha da melhor configuração.

A \autoref{tab:resultados_grid_search} apresenta os 10 RMSE mais baixos e suas configurações:

\begin{table}[H]
	\centering
	\caption{Top 10 configurações de hiperparâmetros ordenadas por RMSE.}
	\label{tab:resultados_grid_search}
	\footnotesize
	\resizebox{\textwidth}{!}{
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\hline
		\textbf{Modelo} & \textbf{Cycle} & \textbf{Modelo} & \textbf{use\_} & \textbf{use\_} & \textbf{use\_vol\_} & \textbf{RMSE} \\
		\textbf{Vazão} & \textbf{Logic} & \textbf{Volume} & \textbf{diff1} & \textbf{diff2} & \textbf{clim} & \\
		\hline
		KNeighborsRegressor() & damping\_specific & LinearRegression() & True & True & False & 2,121061 \\
		\hline
		KNeighborsRegressor() & damping\_specific & Ridge() & True & True & False & 2,133695 \\
		\hline
		RandomForestRegressor() & damping\_specific & Ridge() & True & False & False & 2,165826 \\
		\hline
		RandomForestRegressor() & damping\_specific & LinearRegression() & True & True & False & 2,166875 \\
		\hline
		KNeighborsRegressor() & damping\_specific & LinearRegression() & True & False & False & 2,168468 \\
		\hline
		KNeighborsRegressor() & damping\_specific & Ridge() & True & False & False & 2,178753 \\
		\hline
		KNeighborsRegressor() & damping\_specific & LinearRegression() & False & False & False & 2,237026 \\
		\hline
		KNeighborsRegressor() & damping\_specific & LinearRegression() & False & True & False & 2,238890 \\
		\hline
		RandomForestRegressor() & damping\_specific & LinearRegression() & True & False & False & 2,242538 \\
		\hline
		KNeighborsRegressor() & damping\_specific & Ridge() & False & False & False & 2,243728 \\
		\hline
	\end{tabular}
	}
...
Em uma análise rápida, podemos notar que para termos um melhor modelo, utilizar damping, diff1, diff2 e modelo para o ciclo da vazão foram as melhores configurações. Para o modelo de ciclo da vazão, a utilização do \acrshort{KNN} ou RandomForest não afeta muito, mas o \acrshort{KNN} foi treinado muito mais rápido e na geração de previsões ambos são quase iguais.

Com essa tabela, podemos ver também que a média dos \acrshort{RMSE} é na faixa de 2,1 a 2,2 para essas configurações que comentei acima, desta forma, podemos extrapolar que o modelo pode errar próximo de 5 e 6 nas suas previsões.

Abaixo temos alguns gráficos com a média das previsões para determinados parâmetros, a fim de identificar os que mais afetam e o melhor modelo. Vale notar, que essa análise foi feita para os dados finais de validação, estes gráficos ajudam a dar uma visão de separação máxima de erro (descolamento entre curvas).  

\begin{figure}[H]
	\centering
	\caption{Gráfico comparativo das previsões usando ou não o parâmetro de amortecimento do valor da vazão com o valor real.}
	\label{fig:Fig_27}
	\includegraphics[width=0.90\textwidth,keepaspectratio]{figuras/imagem27.png}
	\fonte{Elaborado pelos autores.}
\end{figure}

\begin{figure}[H]
	\centering
	\caption{Gráfico comparativo das previsões usando ou não o parâmetro de diferença de primeira ordem do volume com o valor real.}
	\label{fig:Fig_28}
	\includegraphics[width=0.90\textwidth,keepaspectratio]{figuras/imagem28.png}
	\fonte{Elaborado pelos autores.}
\end{figure}

\begin{figure}[H]
	\centering
	\caption{Gráfico comparativo das previsões usando ou não o parâmetro de diferença de segunda ordem do volume com o valor real.}
	\label{fig:Fig_29}
	\includegraphics[width=0.90\textwidth,keepaspectratio]{figuras/imagem29.png}
	\fonte{Elaborado pelos autores.}
\end{figure}

\begin{figure}[H]
	\centering
	\caption{Gráfico comparativo das previsões usando ou não o parâmetro de climatologia do volume com o valor real.}
	\label{fig:Fig_30}
	\includegraphics[width=0.90\textwidth,keepaspectratio]{figuras/imagem30.png}
	\fonte{Elaborado pelos autores.}
\end{figure}

Dos gráficos, pode-se observar que a utilização do damping aproxima muito mais do caso real (pode ser por conta de um cenário de baixa de volume, mas se for, com o tempo, nossas métricas indicarão necessidade de retreino devido o ciclo de vida do modelo). Além disso, os outros fatores parecem afetar muito pouco na aproximação do valor real do modelo, pelo menos para esses dados de validação, mas sabemos que fisicamente são parâmetros que podem causar influência significativa.  

\section{Viabilidade Socioeconômica do Projeto}

A viabilidade de um projeto de Machine Learning aplicado à gestão de recursos hídricos vai além da análise financeira tradicional. O abastecimento de água é um ativo crítico de segurança nacional e estabilidade socioeconômica. Neste contexto, a ferramenta desenvolvida atua como um sistema de suporte à tomada de decisão, cuja principal proposta de valor é a antecipação de cenários críticos, permitindo a transição de uma gestão reativa baseada em gerenciamento de crise, para uma gestão preditiva visando a mitigação de riscos.

Embora a mensuração exata do retorno sobre o investimento envolva variáveis que são difíceis de discretizar em valores, a viabilidade socioeconômica do projeto justifica-se pela análise do custo de oportunidade e do custo de mitigação de desastres. A capacidade de antever níveis críticos do \acrshort{SC} gera valor em quatro pilares fundamentais:

\begin{itemize}
\item \textbf{Proteção Social}: A prevenção de colapsos no abastecimento protege prioritariamente as populações em situação de vulnerabilidade social, historicamente as mais afetadas por racionamentos e pela falta de saneamento básico. A continuidade do serviço é, portanto, uma medida de saúde pública e dignidade humana.

\item \textbf{Segurança Sanitária}: Ao evitar o esgotamento do volume útil, minimiza-se a necessidade de captação do volume morto, que exige processos de tratamento mais complexos e onerosos, além de apresentar riscos elevados de concentração de poluentes e metais pesados.

\item \textbf{Economia (Indústria e Agronegócio)}: A água é insumo primário para a indústria de transformação e para a agricultura. A previsibilidade de oferta permite que esses setores planejem suas operações, evitando paradas de produção que impactam o PIB regional e a empregabilidade.

\item \textbf{Geração de energia}: Dada a dependência da matriz energética brasileira em relação às hidrelétricas, a gestão eficiente dos reservatórios impacta diretamente o custo da energia. Evitar o acionamento de bandeiras tarifárias vermelhas (usinas termelétricas) resulta em economia direta para toda a sociedade.
\end{itemize}

Dito isso, é importante ressaltar que o modelo proposto não se destina ao usuário final, mas serve como uma ferramenta de inteligência para a operadora do sistema, no caso do Cantareira, o \acrshort{SABESP}, pois ela é a responsável e possui conhecimento técnico sobre as ações que pode e deve tomar (a disponibilização a um usuário comum poderia levar a cenários catastróficos de ``desespero'' em caso de previsão de falta de abastecimento).

Ao fornecer insights baseados em dados, o projeto empodera os gestores para implementar medidas graduais de economia de água, evitando choques abruptos no abastecimento. Desta forma, não há necessidade de sistemas e servidores robustos para múltiplos acessos ou múltiplas requisições de informações, em nossa construção, estimamos que a informação (dados de previsão e alerta de possível criticidade futura) deva ser repassada via canais oficiais (como email) em forma de relatório, sem necessidade de disponibilizar uma ferramenta interativa.

Do ponto de vista financeiro, a arquitetura do projeto foi desenhada sob os princípios de computação em nuvem e arquitetura Serverless, o que elimina a necessidade de altos investimentos iniciais em hardware, convertendo os custos para um modelo operacional de pagamento por uso (\acrfull{OPEX}). Realizou-se uma simulação de custos operacionais utilizando a calculadora oficial do \acrshort{AWS}, considerando os recursos de armazenamento, computação e banco de dados utilizados na solução final. A \autoref{fig:Fig_31} mostra uma estimativa inicial do custo mensal que desconsidera a quantidade de \acrfull{ACU}s do banco de dados Aurora. Como esse banco consegue reduzir provisionamento na ausência de conexões, para melhor estimativa, foi necessário calcular separadamente.  

\begin{figure}[H]
	\centering
	\caption{Estimativa de Custos Operacionais na Calculadora da AWS.}
	\label{fig:Fig_31}
	\includegraphics[width=0.90\textwidth,keepaspectratio]{figuras/imagem31.png}
	\fonte{Elaborado pelos autores.}
\end{figure}

A estimativa considerou o banco de dados Aurora ativo durante 36 horas por mês (5\% de utilização). Para encontrar este valor, foi assumido que a pipeline executa diariamente, com duração de 30 minutos quando não é necessário retreinar o modelo e 3 horas quando há necessidade. No melhor cenário, em que não é necessário executar retreino durante um mês inteiro, são 15 horas por mês (2,08\%). No pior cenário em que é necessário executar retreino todos os dias, e levando em consideração que a métrica que define essa necessidade é semanal, seriam 4 treinos por mês, resultando em 25 horas (3,47\%). O valor obtido foi arredondado para cima, para ter uma margem de segurança na estimativa. 

O custo de 1 ACU verificado a partir da calculadora foi de US\$ 87.60 dólares, sendo que 5\% de utilização corresponde a US\$ 4,38 dólares. Somados aos US\$ 20,12 dólares da estimativa inicial da figura apresentada, resulta em um custo mensal de aproximadamente US\$ 24,50 dólares, desconsiderando impostos. 

Ao confrontar o custo operacional mensal na ordem de centenas de reais com os prejuízos potenciais de uma crise hídrica que podem chegar a ordem de bilhões de reais, considerando perdas industriais, custos de saúde e tratamento de água emergencial, a relação custo-benefício apresenta-se favorável. O custo de manutenção do sistema é mínimo comparado ao valor econômico e social dos ativos que ele visa proteger, comprovando a alta viabilidade e a sustentabilidade financeira do projeto a longo prazo.
